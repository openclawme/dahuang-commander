    ...
    wx.request({
      ...
      success: (res) => {
        ...
        if (res.statusCode === 200 && res.data && res.data.success && res.data.url) {
          // 复制京东直链。免密码路径（实测后二选一固化）：
          // ① 微信聊天里粘贴发送并点开——微信浏览器里京东通常有登录态，直接「确认授权」
          // ② 手机浏览器打开——京东登录页选「本机号码一键登录」（运营商免密，无需输密码）
          wx.setClipboardData({
            data: res.data.url,
            success: () => {
              wx.showModal({
                title: "授权链接已复制",
                content: "扫码免密流程（已验证）：\n① 打开浏览器（系统会提示打开复制的链接）→ 登录页选「二维码登录」\n② 京东App「扫一扫」→「相册」选二维码截图 → 确认登录（提示写「确认登录电脑端」属正常）\n③ 浏览器出现「确认授权」→ 点确认即完成\n\n备选：登录页用短信验证码。完成后回到本页，状态自动刷新。",
                confirmText: "知道了",
                showCancel: false,
