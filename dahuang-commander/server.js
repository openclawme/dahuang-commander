...
app.use((req, res, next) => {
  res.header('Access-Control-Allow-Origin', req.headers.origin || 'http://localhost:9090');
  res.header('Access-Control-Allow-Methods', 'GET, POST, OPTIONS');
  res.header('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Commander-Secret');
...
app.post(['/client/log', '/commander/client/log'], (req, res) => {
  const secret = process.env.COMMANDER_WEBHOOK_SECRET;
  if (secret) {
...
server.listen(PORT, HOST, () => {
  ...
  console.log(`🔗 网页端访问地址: http://${HOST}:${PORT}`);
  console.log(`🚀 外部 Agent 远程 Webhook 投递点: http://${HOST}:${PORT}/client/log`);
  console.log(`==================================================\n`);
});
